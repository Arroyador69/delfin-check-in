...
export async function POST(req: NextRequest) {
  try {
    console.log('🎯 Test MIR usando configuración que funcionó (Status 404)...');
    ...
    // Generar autenticación (igual que funcionó antes)
...
<solicitud>
  ...
  <comunicacion>
    <contrato>
      <referencia>WORKING-TEST-${Date.now()}</referencia>
    ...
    <persona>
      ...
      <correo>test@working.com</correo>
      <direccion>
        <direccion>Calle Working 123</direccion>
    ...
    // Generar SOAP envelope exactamente como en el test que funcionó
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
  ...
  <soapenv:Body>
    ...
    console.log('📤 Enviando al MIR con configuración que funcionó...');

    // Configuración exacta que funcionó antes (obtuvo Status 404)
    ...
    // Deshabilitar verificación SSL (como funcionó antes)
    ...
    return NextResponse.json({
      success: true,
      message: 'Test MIR con configuración que funcionó completado',
      configuracion: {
        url: config.baseUrl,
        username: config.username,
        codigoArrendador: config.codigoArrendador,
      ...
      interpretacion: {
        ...
        recomendacion: res.status === 404 ? 
          'El endpoint puede estar mal configurado. Verificar documentación MIR.' :
          res.ok ? 
    ...
    console.error('❌ Error en test MIR working:', error);
    
    return NextResponse.json({
      success: false,
      error: 'Error en test MIR working',

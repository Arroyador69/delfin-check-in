...
export async function POST(req: NextRequest) {
  try {
    console.log('🚀 Test conexión MIR desde producción online...');
    
    // Configuración usando las variables de entorno de Vercel
    const config = {
      baseUrl: process.env.MIR_BASE_URL || 'NO CONFIGURADO',
    ...
    if (!config.username || !config.password || !config.codigoArrendador) {
      return NextResponse.json({
        ...
        configuracion: {
          username: config.username ? '***CONFIGURADO***' : 'NO CONFIGURADO',
          password: config.password ? '***CONFIGURADO***' : 'NO CONFIGURADO',
          codigoArrendador: config.codigoArrendador || 'NO CONFIGURADO'
        }
      }, { status: 400 });
    }
    ...
    const testData = {
      ...
      comunicaciones: [{
        contrato: {
          referencia: `TEST-ONLINE-${Date.now()}`,
    ...
    console.log('📤 Enviando test a MIR desde producción online...');
    ...
    console.log('✅ Resultado MIR desde producción online:', resultado);
    
    return NextResponse.json({
      success: true,
      message: 'Test conexión MIR desde producción online completado',
      ...
      interpretacion: {
        ...
        recomendacion: resultado.ok ? 
          '✅ Conexión exitosa - Sistema funcionando en producción' :
    ...
    console.error('❌ Error en test MIR desde producción online:', error);
    
    return NextResponse.json({
      success: false,
      error: 'Error en test MIR desde producción online',

  ...
  const generarDia = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(j?.error || 'No se pudo generar el XML del día');
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Generar XML del día</h1>
        <div className="bg-white rounded-lg shadow p-6 flex items-end gap-4">
          ...
          <button onClick={generarDia} disabled={loading} className="px-6 py-2 bg-blue-600 text-white rounded-md disabled:opacity-50">{loading ? 'Generando…' : 'Generar XML día'}</button>
        </div>
      </div>
    </div>
  );
